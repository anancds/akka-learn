      ...
      post {
        ...
        get {
          ...
          onSuccess(getEvent(event)) {
            val err = Error(s"$event event not found.")
            _.fold(complete(NotFound, err))(e => complete(OK, e))
          }
        } ~
        delete {
          // DELETE /events/:event
      ...
      pathEndOrSingleSlash {
        ...
        entity(as[TicketRequest]) { request => //<co id="ch02_unmarshall_ticket"/>
          onSuccess(requestTickets(event, request.tickets)) { tickets =>
            val err = Error(s"$event event not found.")
            if (tickets.entries.isEmpty) complete(NotFound, err) //<co id="ch02_notfound_if_empty"/>
